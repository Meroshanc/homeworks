...
void calcNextState(bool**& arrayCur, bool**& arrayNext, int sizeX, int sizeY)
{
    for (int rws = 0; rws < sizeY; rws++)
    {
        for (int cols = 0; cols < sizeX; cols++)
        {
            ...
            {
                if (rws + curRow < 0 || rws + curRow >= sizeY) continue;
                for (int curCol = -1; curCol < 2; curCol++)
                {
                    if (cols + curCol < 0 || cols + curCol >= sizeX) continue;
                    if (curRow == 0 && curCol == 0) continue;
                    if (arrayCur[rws + curRow][cols + curCol]) cellsAliveNearby++;
                }
            }
            if(cellsAliveNearby == 3) arrayNext[rws][cols] = true;
            else if(cellsAliveNearby < 2 || cellsAliveNearby > 3) arrayNext[rws][cols] = false;
        }
    }
}
void callTheGame()
